...
function Cart () {
  ...
  React.useEffect(() => {
    window.addEventListener('click', () => {
      ...
      var coordinatesTemp = JSON.parse(
        localStorage.getItem('coordinatesEating')
      )
  ...
  const checkout = () => {
    ...
    var submitOrderInformation = {
      longitude: JSON.parse(localStorage.getItem('coordinatesEating'))
        .longitude,
    ...
    fetch(
      API_GET_USER +
        'cart/submitOrder/' +
        JSON.stringify(submitOrderInformation),
      requestOptions
    )
      .then(response => response.json())
      .then(response => {
  ...
  return (
    <div>
      ...
      <div className='container'>
            ...
            {foodCategories.map(category => (
              <>
                ...
                <div className='flex-row'>
                  {food
                    .filter(
                      current_food => current_food.foodCategory.name == category
                    )
                    .map(foodInfo => (
                      <CartFoodItem
                        key={foodInfo}
        ...
        <Card sx={{ minWidth: 345, my: 20 }}>
          <CardContent>
            <GoogleMaps sx={{ minWidth: 345 }} />
        ...
        <Card sx={{ minWidth: 345, marginRight: 10, marginTop: 5 }}>
          <CardActions>
            <Button size='Big' onClick={clearList}>
              Clear List
            </Button>
          </CardActions>
        </Card>
