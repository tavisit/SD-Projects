...
class SendEmail{
    static sendToClient(information){
        ...
        send(api_key.SERVICE_KEY,api_key.TEMPLATE_ID_Client,clientInfo,api_key.PUBLIC_KEY)
        .then((result) => {
        },
        (error) => {
    ...
    static sendToRestaurant(information){
        ...
        send(api_key.SERVICE_KEY,api_key.TEMPLATE_ID_Restaurant,restaurantInfo,api_key.PUBLIC_KEY)
            .then((result) => {
            },
            (error) => {
            alert("An error occurred, Please try again", error.text);
    ...
    static createInformation(storedInformation){
        var information = {
            client:{
                ...
                location:"Longitude: "+storedInformation.submitOrderInformation.longitude+" Latitude:" + storedInformation.submitOrderInformation.latitude,
                special_message:storedInformation.submitOrderInformation.additionalInformation,
